...
export function BookCards({books}: BookCardsProps) {
    return <div className="flex flex-wrap overflow-y-auto">
...
function BookCard({book}: BookCardProps) {
    return <div
        ...
        <h2 className="mb-2 text-lg font-medium text-gray-900">{book.title}</h2>
        <BookAuthors authors={book.authors}/>
        <div className="flex m-4">
            <p className="text-base font-medium text-gray-500">₹{book.price}</p>
...
function BookThumbnail({title, url}: BookThumbnailProps) {
    return <img className="py-4 h-48 w-full object-contain object-center" src={url} alt={title}></img>
}
...
function BookAuthors({authors}: BookAuthorsProps) {
    return <span
        className="m-4 text-base text-gray-700">{authors.reduce((authors, author) => authors + ", " + author, "").slice(2)}</span>;
}
